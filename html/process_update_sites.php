...
if ($context == "live") {
  ...
  # Roger: change rsync host to local directory
  $rsync_host = "/appl/hudson2BabelLink/";
}
...
while($update_site = mysql_fetch_assoc($rs_maps)) {
  ...
  # Rsync update site
  echo "rsync -av --delete $site_plugins_dir $temp_site_dir\n";
  exec("mkdir -p $temp_site_dir; rsync -av --delete $site_plugins_dir $temp_site_dir");

  exec("ls -la $temp_site_dir/*");
  ...
  foreach (glob("*.jar") as $jar_name) {
    ...
    echo "unzip -o -q $temp_site_dir$jar_name\n";
