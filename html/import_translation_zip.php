...
# Riccardo: we are importing from well known bundles, so set to 0
$fuzzy = 0;

# Riccardo: added "classes", as it is run from the html folder
require(dirname(__FILE__) . "/../classes/file/file.class.php");

# Roger: remove json_encoding import
...
# Riccardo: these infos must come from the URL
$PROJECT_ID = getHTTPParameter("projectId");
$VERSION = getHTTPParameter("version");
echo "Handling projectId=" . $PROJECT_ID . " and version=" . $VERSION . "\n";
				...
				if(strlen($line) > 0 && $line[0] != "#" && $line[0] != ";") {
					...
					if(preg_match("/\\\\$/", $line)) {
						...
						if(count($tags) > 1) {
							...
							# Riccardo: translates unicode back into utf-8 via HTML encodings
							$myTranslation = preg_replace("/\\\\u([0-9abcdef]{4})/", "&#x$1;", $tags[1]);
							$myTranslation = html_entity_decode($myTranslation, ENT_NOQUOTES, 'UTF-8');
						#	echo "Doing " . $tags[0] . " with value " . $tags[1];
							echo "\nUnescaped: " . $myTranslation . " with slashes " . addslashes($myTranslation) . "\n";
                     # roger: gugus machen wir aus newlines br
                     #$myTranslation = nl2br($myTranslation);
                     $myTranslation = addslashes($myTranslation);
                     # Roger: ist ok, aber nun erscheinen die Newlines falsch!
                     $myTranslation = str_replace('\\n', "\n", $myTranslation);
							
							# Get the matching string name
							# Roger: removed addslashes
                   ...
                   and trv.value = '" . $myTranslation . "')
							WHERE s.is_active = 1 AND s.non_translatable <> 1 AND s.file_id = " . $file_id . " AND binary s.name = '" . $tags[0] . "'";
                  	echo $SQL. "\n";
                   ...
                   if(!$rs_string) {
                       echo 'ERROR: ' . mysql_error() . "\n";
							...
							if($myrow_string['string_id'] > 0  				# There is an English string   
								 ...
								 # Riccardo: we want all translations 
                        ...
                        # Roger: remove addshlashes
								...
								VALUES (
									...
									" . $language_id . ", 0, '" . $myTranslation . "', $insert_as_fuzzy, 1, " . $USER . ", NOW()
								)";
								mysql_query($SQL, $dbh);
								 echo $SQL . "\n";
							}
                     else {
                        echo "ERROR: String ". $myTranslation . " not found\n";
                     }
