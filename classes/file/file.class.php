...
# Roger: remove json_encode require
		...
		if($_content != "") {
            ...
            if(strlen($line) > 0 && $line[0] != "!" && $line[0] != ";") {
                  ...
                  # Does line end with a \ ?
                        ...
                        #echo "new String: name=$tags[0]; value=$tags[1] \n";

								$String = new String();
								$String->file_id 	= $this->file_id;
								$String->name 		= $tags[0];
								# Roger: make sure we have real utf-8. Get rid of those unicode multibyte 
                        $string_value=preg_replace("/\\\\u(([a-fA-F0-9]{1,4}))/", "&#x\\1;", $tags[1]);
                        $string_value=html_entity_decode($string_value);
                        $String->value = iconv('ISO-8859-1','UTF-8', $string_value);
                        #echo "new String: converted: $String->value \n";
