            ...
            echo "mkdir -p " . $tmp_dir . $dirname; 
				...
				if (strcmp($language_iso, "en_AA") == 0) {
					...
					while (($strings_row = mysql_fetch_assoc($strings_result)) != null) {
						...
						#	 returns the string with quotes fore and aft.  Need to strip them.
							# $tr_string = preg_replace('/^"(.*)"$/', '${1}', json_encode($strings_row['trans']));
							# $tr_string = str_replace('\\\\', '\\', $tr_string);
							# Roger: replace newline char with \n 
                     # old
                     # $tr_string = toescapedunicode($strings_row['trans']);
                     # new
                     $tr_string =   toescapedunicode(preg_replace("/(\r\n|\n|\r)/", "\\n", $strings_row['trans']));
                     # Roger add logic to handle beginning spaces
                     $tr_string = preg_replace('/^\s/', '\ ', $tr_string);
...
function usage() {
	...
	echo "  -t <train_id>: Optional: train to build (mars, luna, kepler, juno, indigo, helios, galileo, ganymede, europa)";
	echo "\n";
}
